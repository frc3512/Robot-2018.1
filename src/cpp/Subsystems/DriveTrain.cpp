 ...
 * Enable controller.
 */
void DriveTrain::Enable() { m_thread.StartPeriodic(0.005); }
...
void DriveTrain::SetGoal(Pose pose) {
    positionProfile.SetGoal(std::hypot(pose.x, pose.y));
    angleProfile.SetGoal(pose.theta);
}

bool DriveTrain::AtReference() const { return m_drivetrain.AtReferences(); }

bool DriveTrain::AtGoal() {
    return positionProfile.AtGoal() && angleProfile.AtGoal();
}
...
void DriveTrain::Debug() {
    std::cout << "Left U: " << Drivetrain.ControllerLeftVoltage()
              << std::endl
              << " Right U: " << Drivetrain.ControllerRightVoltage()
              << std::endl;
