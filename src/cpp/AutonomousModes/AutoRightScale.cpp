    ...
    switch (state) {
        case State::kInit:
            ...
            if (platePosition[kScale] == 'R') {
                ...
                positionProfile.SetGoal(236.5 + 10.0 -
                                                  kRobotLength / 2.0);
            }
...
~                 positionProfile.SetGoal(324.0 - kRobotLength / 2.0);
                  ...
                  state = State::kInitialForward;
                         ...
                         platePosition[kScale] ') {
~                 positionProfile.SetGoal(324.0 - kRobotLength / 2.0);
                  Robot::elevator.SetHeightReference(kScaleHeight);
  
                  state = State::kInitialForward;
              } else if (platePosition[kFriendlySwitch] == 'L' &&
                         platePosition[kScale] f (angleProfile.AtGoal() ||
                autoTimer.Get() >
                    angleProfile.ProfileTimeTotal() + 1.0) {
                Robot::robotDrive.ResetEncoders();
                if (platePosition[kScale] == 'R') {
                    ...
                    positionProfile.SetGoal(56.0 + 22.0 -
                                                      kRobotLength / 2.0);
                }
        ...
        case State::kPrepReverse:
            if (autoTimer.HasPeriodPassed(1.0)) {
                if (platePosition[kScale] == 'R') {
                    positionProfile.SetGoal(-24.0 - 33.0 +
                                                      kRobotLength / 2.0);
                } else {
                    positionProfile.SetGoal(-56.0 - 9.0 - 12.0 +
                                                      kRobotLength / 2.0);
                }
                autoTimer.Reset();
...
~                 positionProfile.SetGoal(324.0 - kRobotLength / 2.0);
                  ...
                  state = State::kInitialForward;
                         ...
                         platePosition[kScale] ngleProfile.SetGoal(-90.0);
