...
/**
 * Mathematical model representation of a coupled (tank / differential)
 * drivetrain.
 *
 * Chassis contains members for the transmissions (motors), as well as other
 * configurations regarding the chassis (track radius, wheel radius, mass, etc).
 *
...
public:
     ...
     *
     * @param transmission_left   The left side transmission, held internally as
     * a reference.
     * @param transmission_right  The right side transmission, held internally
     * as a reference.
     * @param wheel_radius        The wheel radius, in metres. Should be
     * emperically measured for best performance. Note that both the left and
     * right transmission wheels must be the same
     * radius.
     * @param track_radius        The track radius, a.k.a half the chassis
     * width, in metres. Measured as half the distance between the centres of
     * the left and right transmissions at their
     * points of contact with the ground.
     * @param mass                The mass of the chassis, in kilograms.
     */
    chassis(transmission_t& transmission_left,
            transmission_t& transmission_right, double wheel_radius,
    ...
    /**
     * Calculate the absolute linear (translational) velocity limit of the
     * chassis in metres per second (ms^-1).
     *
     * This calculation relates purely to the free-speed of the motors, meaning
     * for a fully constrained calculation, @ref
     * acceleration_limits(configuration_state&, double, double) should be
     * called and used to constrain the velocity if necessary.
     *
     * @param config    The configuration of the chassis
     * @param curvature The instantaneous curvature, in metres^-1, expected of
     * the chassis, such as the curvature of a path being followed.
     *
     * @return  The absolute linear (translational) velocity limit in metres per
     * second (ms^-2).
     */
    double linear_vel_limit(const configuration_state& config,
                            double curvature) const {
        // Infinite curvature, point turn (purely angular), therefore no linear
        // velocity.
        ...
        Eigen::Vector2d maximum_vels{
            _trans_left.get_free_speed(_trans_left.nominal_voltage()) *
                _wheel_radius,
            _trans_right.get_free_speed(_trans_right.nominal_voltage()) *
                _wheel_radius};

        if (std::abs(curvature) < constants::epsilon) {
            // No curvature, straight forward (purely linear). Need to take
            // minimum as for differing transmissions there may be a non-equal
            // speed limit. We can skip all the logic below to be more
            // efficient. We return eagerly.
            return maximum_vels.minCoeff();
        } else {
            // Take the maximum speed of each wheel, then limit the opposing
            // one, ensuring we are within the limits above.
            ...
            // k = w / v_c                          [3] Curvature
            // note w / v_c has 0.5 as common factor, cancels out for the
            // following k = ((v_r - v_l)/r) / (v_r + v_l) rk = (v_r - v_l) /
            // (v_r + v_l) v_r(1 - rk) = v_l(1 + rk)            [4] let ratio =
            // (1 - rk) / (1 + rk)
            double ratio = (1 - _track_radius * curvature) /
                           (1 + _track_radius * curvature);
            // v_l = v_r * ratio                    [5]
            // v_r = v_l / ratio                    [6]

            // TODO: This can all be compressed using some clever linalg
            if (curvature > 0) {
                // Banking counter-clockwise (steering to the left, right side
                // dominant) therefore, v_r = maximum freespeed
                wheel_vels[1] = maximum_vels[1];
                // v_l = v_r * ratio                  via [5]
                wheel_vels[0] = wheel_vels[1] * ratio;

                if (wheel_vels[0] > maximum_vels[0]) {
                    // Left side is over it's maximum speed - cap it to its
                    // maximum speed, then limit the right side.
                ...
                if (wheel_vels[0] > maximum_vels[0]) {
                    // Right side is over it's maximum speed - cap it to its
                    // maximum speed, then limit the left side.
                    wheel_vels[1] = maximum_vels[1];
                    // v_l = v_r * ratio                via [5]
                    wheel_vels[0] = wheel_vels[1] * ratio;
                }
    ...
    /**
     * Calculate the minimum and maximum linear (translational) acceleration
     * limits of the chassis, in metres per second per second (ms^-2).
     *
     * This calculation uses torque limits of the transmissions, meaning speed
     * limits are not directly taken into account. For a fully constrained
     * representation,
     * @ref linear_vel_limit(configuration_state&, double) must also be called
     * and used to constrain if necessary.
     *
     * @param config    The configuration of the chassis
     * @param curvature The instantaneous curvature, in metres^-1, expected of
     * the chassis, such as the curvature of a path being followed.
     * @param velocity  The current linear velocity of the chassis, in metres
     * per second (ms^-1).
     *
     * @return  A pair, ordered [min, max], of the linear acceleration limits,
     * in metres per second per second (ms^-2).
     */
    std::pair<double, double> acceleration_limits(
        const configuration_state& config, double curvature,
        ...
        Eigen::Vector2d fwd_torque_limits{
            _trans_right.get_torque(_trans_right.get_current(
                _trans_right.nominal_voltage(), wheels[0])),
            _trans_left.get_torque(_trans_left.get_current(
                _trans_left.nominal_voltage(), wheels[1]))};

        Eigen::Vector2d fwd_accel_limits =
            fwd_torque_limits / (_mass * _wheel_radius);
        ...
        Eigen::Vector2d rvs_accel_limits =
            rvs_torque_limits / (_mass * _wheel_radius);
    ...
    /**
     * Split a centre state of this chassis into the left and right transmission
     * state components.
     *
    ...
    std::pair<wheel_state, wheel_state> split(const state centre) const {
        ...
        rotation << std::cos(heading), -sin(heading), std::sin(heading),
            std::cos(heading);

        // Rotate the wheel offsets by the heading of the robot, adding it to
        // the centre position, this 'splits' the centre path into two paths
        // constrained by the configuration (heading + position) and track
        // radius.
        ...
        double a_angular = a_linear * centre.curvature +
                           v_linear * v_linear * centre.dcurvature;
...
private:
    ...
    void do_solve_electrical(wheel_state& wheel,
                             transmission_t& transmission) const {
