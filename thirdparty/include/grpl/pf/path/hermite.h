 ...
 *
 * Base implementation of a hermite spline, either quintic or cubic, as
 * detemined by the ORDER template parameter. This class should not be used
 * directly, instead see @ref hermite_cubic and @ref hermite_quintic.
 *
...
public:
     ...
     * the columns are for the start point, and the second half for the end
     * point. The columns are ordered position, tangent for cubic, and position,
     * tangent, dtangent for quintic.
     *
     * @param M The control matrix.
     */
    explicit hermite(control_matrix_t& M) { set_control_matrix(M); }
    ...
    double curvature(double t) override {
        ...
        return (h_p[0] * h_pp[1] - h_p[1] * h_pp[0]) / std::pow(h_p.norm(), 3);
    }
 ...
 *
 * The cubic spline is defined in regards to its waypoints, which are defined in
 * terms of position and tangent.
 */
...
protected:
    inline basis_t basis(double t) const override {
        ...
        // -2t^3 + 3t^2
        // t^3 - t^2
        return (basis_t() << (2 * t * t * t - 3 * t * t + 1),
    ...
    inline basis_t basis_1st(double t) const override {
        return (basis_t() << (6 * t * t - 6 * t), (3 * t * t - 4 * t + 1),
                (-6 * t * t + 6 * t), (3 * t * t - 2 * t))
            .finished();
    }

    inline basis_t basis_2nd(double t) const override {
        return (basis_t() << (12 * t - 6), (6 * t - 4), (-12 * t + 6),
                (6 * t - 2))
            .finished();
    }
};
 ...
 *
 * The quintic spline is defined in regards to its waypoints, which are defined
 * in terms of position, tangent and the derivative of the tangent.
 */
...
public:
    ...
    struct waypoint {
        ...
        //! 2D tangent to the waypoint, in metres. Ordered x, y.
        vector_t tangent;
        //! 2D derivative of the tangent to the waypoint, in metres. Ordered x,
        //! y.
        vector_t dtangent;
    };
     ...
     * @param end   The waypoint of the end of the spline.
     */
    hermite_quintic(waypoint& start, waypoint& end) {
        set_waypoints(start, end);
    }
...
protected:
    inline basis_t basis(double t) const override {
        ...
        // 7t^4 - 4t^3 - 3t^5
        // 0.5t^3 - t^4 + 0.5t^5
        return (basis_t() << (1 - 10 * t * t * t + 15 * t * t * t * t -
                              6 * t * t * t * t * t),
    ...
    inline basis_t basis_1st(double t) const override {
        return (basis_t() << (-30 * t * t + 60 * t * t * t -
                              30 * t * t * t * t),
...
template <typename hermite_t, typename output_iterator_t,
          typename iterator_wp_t>
size_t generate(const iterator_wp_t wp_begin, const iterator_wp_t wp_end,
    ...
    if (max_size < std::distance(wp_begin, wp_end))
