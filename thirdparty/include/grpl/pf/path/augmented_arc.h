 ...
 *
 * The Augmented Arc is an arc with a non-continuous curvature, slightly
 * disobeying the geometry of an arc segment. This class is used as the output
 * of an approximation done by @ref arc_parameterizer designed to approximate
 * splines with continous curvature.
 *
 * The curvature is interpolated with respect to the arc length of the segment.
 * This is necessary for certain systems that require a parameterized spline.
 */
...
public:
    ...
    /**
     * Create a circular arc from a set of 3 points (start, any, and end), and
     * the start and end curvature.
     *
     * @param start   The start point of the curve, in x,y metres.
     * @param mid     Any point along the curve sitting between start and end,
     *                in x,y metres.
     ...
     */
    augmented_arc2d(vector_t start, vector_t mid, vector_t end, double start_k,
                    double end_k)
        : arc2d(start, mid, end) {
