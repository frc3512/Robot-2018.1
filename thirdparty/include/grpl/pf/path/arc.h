 ...
 *
 * Implementation of a 2-Dimensional circular arc, parameterized to arc length
 * 's'.
 */
...
public:
    ...
    vector_t position(const double s) const override {
        ...
        if (curv != 0) {
            ...
            return _ref + vector_t{cos(angle) / std::fabs(curv),
                                   std::sin(angle) / std::fabs(curv)};
        } else {
    ...
    vector_t derivative(const double s) const override {
        ...
        if (curv != 0) {
            ...
            return vector_t{cos(angle + off), std::sin(angle + off)};
        } else {
            return _delta;
        }
    }
...
private:
    void from_three(vector_t start, vector_t mid, vector_t end) {
        ...
        if (coeffmatrix.determinant() == 0) {
            ...
            _delta = end - start;
            _length = _delta.norm();
            _angle_offset = std::atan2(_delta[1], _delta[0]);
        } else {
            ...
            _angle_offset = std::atan2((start - _ref)[1], (start - _ref)[0]);
            double angle1 = std::atan2((end - _ref)[1], (end - _ref)[0]);

            _curvature = 1.0 / (start - _ref).norm();
            _length = std::fabs((angle1 - _angle_offset)) / _curvature;
