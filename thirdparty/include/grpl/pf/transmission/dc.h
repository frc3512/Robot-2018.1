 ...
 * Motor transmissions.
 *
 * The grpl::pf::transmission namespace contains implementations of
 * transmissions, which convert control signals to motion (e.g. motors).
 */
namespace transmission {

/**
 * Base class of a DC electric transmission, usually @ref
 * grpl::pf::transmission::dc_motor.
 */
...
public:
    ...
    /**
     * Calculate the free (no load) speed of the transmission at an applied
     * voltage.
     *
    ...
    /**
     * Calculate the current draw of the transmission at an applied voltage and
     * speed.
     *
     * @param voltage The voltage applied to the transmission, in Volts
    ...
    /**
     * Calculate the component voltage applied to the transmission in order to
     * obtain a free speed.
     *
     * To obtain the full applied voltage, sum @ref get_free_voltage(double)
     * const and
 ...
 *
 * Basic DC Motor Model, dervied from the ideal resistive motor model with Back
 * EMF
...
public:
     ...
     * @param free_speed    The maximum free (no load) speed of the motor at
     * v_nom, in rad/s
     * @param free_current  The current drawn when the motor is running at
     * free_speed with no load
     * @param stall_current The current drawn when v_nom is applied with a
     * locked rotor (stalled)
     * @param stall_torque  The torque applied by the motor at v_nom with a
     * locked rotor (stalled)
     */
    dc_motor(double v_nom, double free_speed, double free_current,
             double stall_current, double stall_torque)
        : _v_nom(v_nom),
     ...
     */
    inline double internal_resistance() const {
        return _v_nom / _stall_current;
    }
    ...
    inline double kv() const {
        return (_v_nom - _free_current * _v_nom / _stall_current) / _free_speed;
    }
