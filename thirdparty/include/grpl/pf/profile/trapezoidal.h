 ...
 *
 * A trapezoidal motion profile is a motion profile limited by acceleration
 * (therefore, infinite jerk). The profile can be described by three distinct
 * sections: ramp-up, hold and ramp-down.
 *
 * During ramp-up, the system is accelerating towards its max velocity.
 *
 * During hold, the system is not accelerating, and holds its max velocity.
 * Depending on the setpoint, the system may not have time to reach hold before
 * it must ramp-down, resulting in a trianglular velocity profile.
 *
...
public:
    ...
    state calculate(state& last, double time) override {
        ...
        for (int i = 1; i <= slice_count; i++) {
            ...
            // TODO: Find point at which we reach v_max and if it's less than
            // dt, split this slice into half.
            double v_projected = kin[VELOCITY] + accel * dt;
            v_projected = v_projected > vel_max
                              ? vel_max
                              : v_projected < vel_min ? vel_min : v_projected;

            double decel_time = v_projected / -accel_min;
            double decel_dist = v_projected * decel_time +
                                0.5 * accel_min * decel_time * decel_time;
            double decel_error = kin[POSITION] + decel_dist - this->_goal;

            // TODO: make this better
            // If we decelerate now, do we cross the zero of the error function?
            bool decel_cross_error_zeros = (error > 0 && decel_error < 0) ||
                                           (error < 0 && decel_error > 0);
            // Are we not currently decelerating?
            bool decel_not_in_progress = (error < 0 && kin[VELOCITY] > 0) ||
                                         (error > 0 && kin[VELOCITY] < 0);

            if (decel_cross_error_zeros && decel_not_in_progress)
                accel = (accel < 0 ? accel_max : accel_min);
            else if (std::fabs(kin[1] - vel_max) <
                     constants::default_acceptable_error)
                accel = 0;
            else if (std::fabs(error) < constants::default_acceptable_error)
                accel = 0;

            double vel = kin[VELOCITY] + (accel * dt);
            kin[POSITION] =
                kin[POSITION] + (kin[VELOCITY] * dt) + (0.5 * accel * dt * dt);
            kin[VELOCITY] =
                vel > vel_max ? vel_max : vel < vel_min ? vel_min : vel;
